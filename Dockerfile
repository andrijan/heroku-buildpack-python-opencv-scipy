...
RUN ../configure -b 64 -D c -DPentiumCPS=2400 \
     ...
     --with-netlib-lapack-tarfile=/app/.heroku/lapack-3.7.1.tgz
RUN make build && make check && make ptcheck && make time && make install
WORKDIR /app/.heroku
RUN rm lapack-3.7.1.tgz
...
RUN curl -s -L https://www.python.org/ftp/python/2.7.13/Python-2.7.13.tgz > Python-2.7.13.tgz
RUN tar zxvf Python-2.7.13.tgz
RUN rm Python-2.7.13.tgz
WORKDIR /app/.heroku/Python-2.7.13
...
# Install Numpy
RUN pip install -v numpy==1.13.0


# Install Scipy
RUN pip install -v scipy==0.19.1


# Install Matplotlib
RUN pip install -v matplotlib==2.0.2


# Install Opencv with python bindings
RUN apt-get install -y cmake
RUN curl -s -L https://github.com/opencv/opencv/archive/2.4.13.2.zip > opencv-2.4.13.2.zip
RUN unzip opencv-2.4.13.2.zip
RUN rm opencv-2.4.13.2.zip
WORKDIR /app/.heroku/opencv-2.4.13.2
